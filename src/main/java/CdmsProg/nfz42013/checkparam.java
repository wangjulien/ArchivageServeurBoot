...
public class checkparam extends MainObject {
	...
	public boolean Exec() throws CdmsException {
		 ...
		 * st.executeQuery("select cryptage from param where paramid=1"); if (rs.next())
		 * { if (rs.getObject("cryptage").equals(CurrentObjectValues.get("cryptage"))) {
		 * rs.close(); st.close(); return true; } } rs.close(); rs =
		 * st.executeQuery("select count(*) from document"); if (rs.next()) { if
		 * (rs.getInt(1)>0) { rs.close(); st.close();
