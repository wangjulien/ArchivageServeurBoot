package com.telino.avp.CdmsProg.nfz42013;
...
public class checkpar_doctypes extends MainObject {
	...
	static final Logger logger = Logger.getLogger(com.telino.avp.CdmsProg.nfz42013.checkpar_doctypes.class);
	...
	public boolean Exec() throws CdmsException {
		...
		try {
		PreparedStatement read = conn.prepareStatement("select ar_profile from profils where par_id = "+CurrentObjectValues.get("par_id"));
		...
		PreparedStatement readdoctypes = conn.prepareStatement("select a.doctype_archivage, a.categorie, array_agg(e.content_type) as content_types "
				+ "from doctypes a "
				+ "left join mime_doctypes d on a.doctypeid = d.doctypeid "
				+ "join mime_type e on d.mime_type_id = e.mime_type_id "
				+ "where a.doctypeid = "+CurrentObjectValues.get("doctypeid") + " "
				+ "group by a.doctype_archivage, a.categorie");
		...
		if (rs.next()) {
			...
			for (int i = 0; i < content_types.length; i++) {
				if (content_types_String.length()>0) content_types_String+=",";
		...
		if (CurrentObjectValues.get("mode").equals("create")) mode = "création";
		else if (CurrentObjectValues.get("mode").equals("delete")) mode = "Suppression";
		
		String user = CurrentObjectValues.get("$USERID")+"";
		String mailid = CurrentObjectValues.get("$MAILID")+"";
		
//		PreparedStatement insert = conn.prepareStatement("insert into log_archive (operation,horodatage,userid,mailid) values(?,?,?,?)");
		
		
		String action = "Action sur les profils - "
		+ mode + " des documents du profil "+profil 
		+ " valeurs: " + "type de document: "+ doctype + ", " 
		+ "categorie: " + categorie + ", "
		...
		if (!storageService.archive(attestation)) {
			try {
				...
				throw new AvpExploitException("520", e,"Archivage des attestations de "+mode+" de profil d'archivage",null,null,null);
			}
		}
